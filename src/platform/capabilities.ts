...
export const CAPABILITIES: ReadonlyArray<Capability> = Object.freeze([
  ...
  { key: 'club.create', module: 'clubs', label: 'Create club and invite its president', status: 'LIVE', risk: 'CRITICAL',
    note: 'Creates the club PENDING_SETUP and invites a named president. The platform owner is never made the club owner; the club activates only once the invited person accepts.',
    endpoint: 'POST /system/clubs' },
  { key: 'club.president.resend', module: 'clubs', label: 'Resend the president invitation', status: 'LIVE', risk: 'SENSITIVE',
    note: 'Mints a new link, retires the old one immediately, and emails it through the configured provider. With no provider configured the invitation is still minted and the screen says EMAIL DELIVERY NOT CONFIGURED rather than claiming a send.',
    endpoint: 'POST /system/clubs/:clubId/president/resend' },
  { key: 'club.president.revoke', module: 'clubs', label: 'Revoke the president invitation', status: 'LIVE', risk: 'SENSITIVE',
    note: 'The link stops working immediately and the club returns to PENDING_SETUP.',
    endpoint: 'POST /system/clubs/:clubId/president/revoke' },
  { key: 'club.president.replace', module: 'clubs', label: 'Invite a different president', status: 'LIVE', risk: 'CRITICAL',
    note: 'Only while nobody has accepted. Refused outright once a club has an active president — replacing one is a protected club action, not an invitation control.',
    endpoint: 'POST /system/clubs/:clubId/president/replace' },
  { key: 'club.president.change', module: 'clubs', label: 'Replace an ACTIVE president', status: 'NOT_AVAILABLE', risk: 'PROTECTED',
    note: 'Removing somebody\'s authority over their own club is not a SYSTEM button. The last-owner protection in the membership service remains the only path, and it is not reachable from here.' },
  ...
  { key: 'people.invite', module: 'people', label: 'Invite to a club', status: 'LIVE', risk: 'SENSITIVE',
    note: 'A club administrator invites an address; the platform mints a single-use token, emails it through the configured provider and records the delivery separately from the invitation\'s own validity.',
    endpoint: 'POST /invitations' },
  { key: 'people.invite.resend', module: 'people', label: 'Resend invitation', status: 'LIVE', risk: 'SENSITIVE',
    note: 'Mints a new token, retires the old one and emails the new link. Rate-limited per invitation so repeated pressing never reaches the provider.',
    endpoint: 'POST /invitations/:id/resend' },
  { key: 'email.status', module: 'settings', label: 'Inspect email delivery configuration', status: 'LIVE', risk: 'SAFE',
    note: 'Which provider is selected, whether it is configured, where acceptance links point, and the delivery counters. No credential is in the shape it returns.',
    endpoint: 'GET /system/email' },
  ...
  { key: 'analytics.platform', module: 'platform-analytics', label: 'DAU / WAU / MAU, sessions, peaks, retention', status: 'LIVE', risk: 'SAFE',
    note: 'Unique authenticated users over real analytics events, by environment. Retention reports "collecting data" until a cohort is old enough rather than a figure it cannot support.',
    endpoint: 'GET /system/analytics/platform' },
  { key: 'analytics.retention-policy', module: 'platform-analytics', label: 'Analytics retention policy', status: 'LIVE', risk: 'SAFE',
    note: 'Raw events expire, daily rollups are kept. Both configurable through ANALYTICS_RAW_RETENTION_DAYS and ANALYTICS_ROLLUP_RETENTION_DAYS.',
    endpoint: 'GET /system/analytics/platform' },
  { key: 'analytics.product', module: 'product-analytics', label: 'Module usage, adoption, journeys', status: 'LIVE', risk: 'SAFE',
    note: 'Opens, unique users, dwell time, adoption and trend per module, from recorded events. Journeys are module keys only — no page content and nothing anybody typed.',
    endpoint: 'GET /system/analytics/product' },
  { key: 'analytics.ingest', module: 'product-analytics', label: 'Record a usage event', status: 'LIVE', risk: 'SAFE',
    note: 'One authenticated batch endpoint. An unknown event name, an unknown field or a forbidden key is dropped rather than stored.',
    endpoint: 'POST /telemetry/events' },
  { key: 'analytics.export', module: 'product-analytics', label: 'Export raw events', status: 'NOT_AVAILABLE', risk: 'PROTECTED',
    note: 'No raw-event export exists. Aggregates answer the questions SYSTEM asks, and a row-level export is a data transfer that needs its own decision.' },
