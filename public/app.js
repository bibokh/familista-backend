  ...
  el.innerHTML = players.map((p,i) => `
    <div class="card dev-card clickable">
      <div class="dev-hdr">
        ...
        <div>
          <div class="dev-name">${_esc(p.firstName)} ${_esc(p.lastName)}</div>
          <div class="dev-serial">${_esc(p.device?.serialNumber || `FAM-${p.id.slice(-6).toUpperCase()}-2025`)}</div>
          <div class="dev-online"><div class="dev-pulse"></div><span class="dev-status">Online</span></div>
        </div>
        <span class="pos-pill ${posClass(p.position)}" style="margin-left:auto;">${_esc(p.position)}</span>
      </div>
      <div class="dev-stats">
